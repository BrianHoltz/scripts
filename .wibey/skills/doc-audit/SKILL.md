...
> [`relationship-shared/.wibey/skills/doc-audit/`](https://gecgithub01.walmart.com/CatalogRelationships/relationship-shared).
...
Sits at the very top. Always structured bullets — never prose paragraphs. See [ProjectTemplate.md](../../../docs/templates/ProjectTemplate.md) and [Incident.md](../../../docs/templates/Incident.md) for canonical markup.
...
- **Future work appears only in task lists, Open Questions, Draft Next Comms, and TODOs.** No "Critical/Important/Urgent" labels — use order. No capitalized exclamations (Bug, Gap, Pending, Next). No ⚠️ in task tables — see [Cross-References for Pending Work](#cross-references-for-pending-work) for the ⚠️ antipattern rule.
...
- **Open Questions** — unresolved decisions that need an *answer* from your own team — not an action, and not a response from another team. If resolving the question requires doing something, that doing is a Task; the question stays in Open Questions until answered. If a question turns out to need an external answer, *convert* it: remove it from Open Questions, create a Draft Next Comms entry, and mark any blocking Task 🛑. Never leave the question in both places. Every Open Question that blocks a Task must have a corresponding ▶️ (or 🛑) row in the Tasks table. Move to Closed Questions once answered — never delete.
...
| Open Question | `❓ OQ: [question summary](#open-questions)` |
...
- [ProjectTemplate.md](../../../docs/templates/ProjectTemplate.md) — template and inline guidance for new project docs
