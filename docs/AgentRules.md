...
- Ask the user for explicit permission before touching >10 files. This count includes all writes, moves, renames, and deletions, across all directories. Exception: files inside any folder (at any depth) matching these names are exempt — they are agent-owned scratch space or build-tool output where high-volume writes are expected:
  - tmp/
  - target/classes/
