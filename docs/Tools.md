...
- **TypeDown**: has `typedown.editor.fontFamily` and `typedown.editor.fontSize` settings — current matching settings in `settings.json`:
  ...
  ```
- **Zaaack**: uses `markdown-editor.customCss` in settings.json. The CSS rolls together (a) sans-serif + 13px / 1.4 font normalization for the whole vditor surface (preview, IR `pre.vditor-reset`, source `.vditor-sv`) plus an explicit `font-family: inherit` cascade onto `<table>` / `<td>` / `<th>` because vditor's table CSS doesn't otherwise inherit our font setting through every nested IR node, (b) tighter table cell padding (`3px 7px` vs vditor's default `6px 13px`) for MPE-preview-like density, (c) `margin: 0` on `<p>` / `<ul>` / `<ol>` / `<li> > <p>` inside table cells — vditor IR wraps every cell's content in `<p>` with browser-default `1em` margins, which is the main reason cells render unnecessarily tall (same root cause as the TypeDown table fix), (d) inline `<code>` shrunk to `0.92em` with thinner padding so backtick-formatted text doesn't blow out cell heights, and (e) the prior dark-theme color overrides:

  ```json
  "markdown-editor.customCss": ".vditor .vditor-reset, .vditor-ir pre.vditor-reset, .vditor-sv {font-family: -apple-system, BlinkMacSystemFont, 'Segoe WPC', 'Segoe UI', system-ui, 'Ubuntu', 'Droid Sans', sans-serif !important;font-size: 13px !important;line-height: 1.4 !important;}.vditor-reset table, .vditor-reset table td, .vditor-reset table th {font-family: inherit !important;font-size: inherit !important;line-height: 1.4 !important;}.vditor-reset table td, .vditor-reset table th {padding: 3px 7px !important;vertical-align: top !important;}.vditor-reset table td > p, .vditor-reset table th > p,.vditor-reset table td > p:first-child, .vditor-reset table td > p:last-child {margin: 0 !important;}.vditor-reset table td ul, .vditor-reset table td ol {margin: 0 !important;padding-left: 1.2em !important;}.vditor-reset table td li > p { margin: 0 !important; }.vditor-reset code:not(.hljs):not(.highlight-chroma) {font-size: 0.92em !important;padding: 0.05em 0.35em !important;}.vscode-light .vditor--dark .vditor-reset {color: #111111 !important; background: #ffffff !important;}.vscode-light .vditor--dark .vditor-ir pre {color: #111111 !important;}"
  ```

  After changing the setting: in both VS Code and Cursor, reopen the markdown tab in zaaack (or `Developer: Reload Window`) — webview customCss is read at panel creation, not on settings change. **Goal: visually match Markdown Preview Enhanced (`⇧⌘V`) for tables — same column widths, same row density, same font.** This setting is layered on top of the structural patches in `main.css` (dark theme overrides) and `out/extension.js` (theme listener); the customCss approach is the right knob for cosmetic tuning because it survives extension upgrades.
...
- **Runtime CSS injection (`__zaaackInjectStyle`).** Appends a `<style id="__zk-style">` element to `document.head` containing the same rules as the user's `markdown-editor.customCss` setting (sans-serif font stack, 13px / line-height 1.4, tight table-cell padding `3px 7px`, zero `<p>` margins inside cells, smaller inline-code). The customCss setting is read once at panel construction in `_getHtmlForWebview()`; injecting at runtime in the `after()` hook guarantees the rules land in the cascade after both `main.css` and the panel's `<style>` block, so a stale panel HTML or specificity collision can't cause a visual no-op. Idempotent: re-running checks `getElementById('__zk-style')` first.
...
- 2026.05.09 Sat: Fixed the last three Zaaack gaps — Cmd+F find-in-page, outline view, and intra-doc anchor link nav — via `~/bin/patches/patch-zaaack.py` (4 idempotent patch sites in `media/dist/main.js`). Zaaack jumped from `🟡` to `✅✅` parity with IDEA's md preview, and (since the patch globs both `~/.vscode/extensions/` and `~/.cursor/extensions/`) Cursor now matches VS Code on markdown editing too. Upstream is MIT (`github.com/zaaack/vscode-markdown-editor`) but with low maintainer activity (108 open issues, 5+ stale PRs); the anchor-link-opens-directory bug appears unreported, filed as issue #156. Find-in-page (#153) and outline (#24, #155) are already requested upstream. Same day: aligned Zaaack render with the MPE preview (sans-serif font, 13px, tight `3px 7px` table padding, zero `<p>` margins inside cells) by both updating `markdown-editor.customCss` *and* injecting the same rules at runtime via the enhancer (`__zaaackInjectStyle()`) — belt-and-suspenders so a stale panel HTML can't bypass the styling.
